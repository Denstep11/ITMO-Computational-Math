...
public class Degree {
    ...
    public void init(double[][] functionTable) {
        ...
        for (int i = 0; i < functionTable.length; i++) {
            ...
            xy_sum += Math.log(functionTable[i][0]) * Math.log(functionTable[i][1]);
        }
    ...
    public void solve() {
        ...
        Function function = x -> (Math.exp(res[1]) * Math.pow(x, res[0]));

        result = new Result();
        result.setNameFunction("Степенная");
        result.setStrFunction(Math.exp(res[1]) + "x^" + res[0]);
